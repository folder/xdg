...
const load = (filepath, options = {}, resolve = fp => fp) => {
  const { modify, onProperty } = options;
    ...
    for (const line of contents.split(NEWLINE_REGEX)) {
      if (line.trim() !== '' && !line.startsWith('#')) {
        ...
        if (typeof onProperty === 'function') {
          ...
          ({ key, value } = onProperty(key.trim(), value, { name, path, format, resolve }));
        } else if (modify !== false) {
          key = format !== false ? formatKey(key.trim()) : key.trim();
          value = resolve(cast(value));
        }
